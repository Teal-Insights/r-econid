...
```{mermaid}
flowchart TD
    A[Start: standardize_economy] --> B[Validate Input Data]
    B --> C[Process Custom Economies]
    B --> D[Validate Custom Names]
    C --> E[Convert Names to UTF-8]
    D --> E
    E --> F[Map Output Columns]
    F --> G[Standardize Economies]
    G --> H{Show Aggregates}
    H -->|Yes| I[Report Detected Aggregates]
    H -->|No| J[Prepare Output]
    I --> J
    J --> K[Bind with Original Data]
    K --> L[Return Standardized Data]

    %% Standardize Economies Process
    G --> M[Apply Custom Names]
    M --> N[Combine Standard & Custom Patterns]
    N --> O[Process Each Name]
    O --> P{Match Found?}
    P -->|Yes| Q[Store Match Results]
    P -->|No| R[Store Default Values]
    Q --> S[Next Name]
    R --> S
    S --> T{More Names}
    T -->|Yes| O
    T -->|No| U[Identify Unique Aggregates]
```
