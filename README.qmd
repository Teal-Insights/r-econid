...
```{mermaid}
flowchart TD
    A[standardize_economy] --> B[Input Validation]
    B --> C[Convert name column to UTF-8]
    C --> D[Map output columns]
    D --> E[Call standardize_economies_impl]
    E --> F[Initialize results tibble]
    F --> G[Get economy patterns]
    G --> H[Process each name]
    H --> I[Match economy]
    I --> J{Try regex match}
    J -->|Match found| K[Return matched economy info]
    J -->|No match| L[Return default values]
    K --> M[Store results]
    L --> M
    M --> N[Identify aggregates]
    N --> O[Return standardized data]
    O --> P[Report aggregates if requested]
    P --> Q[Prepare final output]
    Q --> R[Return standardized data frame]
```
